 ...
 * @author Keys & findKey by Vinh, Emily, James, Mirhee
 */
public class PitchShift {
	...
	float[] TARGET_PITCHES = {16.35f, 17.32f,18.35f,19.45f,20.60f,21.83f,23.12f,24.50f,25.96f,27.50f,29.14f,
	...
	String[] POSSIBLE_KEYS = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#"};
	...
	public float[] findKey(String key) {
		...
		int notenum = 0;
		int index = 0;
		float[] keyPitches = new float[(7 * TARGET_PITCHES.length)/12];
		for (int i = 0, len = POSSIBLE_KEYS.length; i < len; i++){
			if (key == POSSIBLE_KEYS[i]){
				start = i;
			}
		}
		for (int j = start, len2 = TARGET_PITCHES.length; j < len2; j++){
			if (notenum%7 == 2 || notenum%7 == 6){
				keyPitches[index] = TARGET_PITCHES[j];
				index++;
				notenum++;
			}
			else {
